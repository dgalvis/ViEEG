function [out,out_full] = BNI_find(net_aux,out,out_full,count,params)
...
% ----------------------------------------------------------------------- %
    ...
    % all attempted w values and their resulting BNI arrays
    out_full{count}.w_save = w_save;
    out_full{count}.BNI    = BNI;


    % the final w value is in out{count} and is the w that gives BNI = 0.5
    % (using linear interpolations of the w vals vs. BNI vals
    try
        % interpolate, BNI_single.m can make sure this value is good
        out{count} = interp1(aux(:,1),aux(:,2),0.5);
        disp(['result= ', num2str(out{count})]);
    catch
        disp('nan');
        out{count} = nan;
    end
