...
function miout=MIhigherdim(x,kneig,emb_dim,emb_tau, idx)
...
save(['zwspMIhigh', num2str(idx),'.txt'], 'zwsp', '-ASCII');


% execute C Programm
[a unout]=unix(['./MIhigherdim zwspMIhigh', num2str(idx),'.txt ',num2str(Nd),' ',num2str(emb_dim),' ',num2str(emb_tau),' ',num2str(N),' ',num2str(kneig)]);
miout=str2num(unout);
delete(['zwspMIhigh', num2str(idx),'.txt']);


miout = sqrt(1 - exp(-2*miout));
end
